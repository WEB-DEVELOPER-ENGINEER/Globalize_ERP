...
import { MonthlyClosing, StaffLiability, UserRole } from '../types';
...
export const MonthlyClosingPage: React.FC<MonthlyClosingPageProps> = ({ isRtl, currentRole }) => {
  ...
  const [rateAed, setRateAed] = useState<number>(14);
  const [rateUsd, setRateUsd] = useState<number>(50.88);
  ...
  const handlePerformClosing = (e: React.FormEvent) => {
    ...
    if (currentRole !== 'owner') {
      alert('Access Denied. Only Owners can lock and authorize monthly closes.');
      return;
    }
    ...
    try {
      ...
      setClosingNotes('');
      alert(`Period ${targetMonth} closed and locked successfully. Ahmed and Abu El-Fotouh shares registered.`);
    } catch (err: any) {
      alert(err.message || 'Error occurred during closing.');
    }
  };

  const handleSettleLiab = (liabId: string, fullRemaining: number) => {
    if (currentRole !== 'owner' && currentRole !== 'accountant') {
      alert('Access Denied. Only Owners or Accountants can sign payout vouchers.');
      return;
    }
  ...
  return (
    <div className="space-y-6 font-sans text-slate-700">
          ...
          <form onSubmit={handlePerformClosing} className="space-y-4 text-xs">
            ...
            {currentRole === 'owner' ? (
              ...
              <div className="p-3 bg-slate-50 border border-slate-250 border-slate-200 rounded-xl text-[10px] font-semibold text-slate-400 flex items-center gap-1.5">
                ...
                <span>Only simulated OWNER has locking access.</span>
              </div>
            )}
          </form>
          ...
          <table className="w-full text-xs text-left text-slate-600 font-sans border-collapse">
            <thead className="bg-[#1B4F72] text-white">
              <tr>
                ...
                <th className="px-4 py-2 font-bold text-right border-r border-[#153e5a]">Ahmed Share</th>
            ...
            <tbody className="divide-y divide-slate-100 font-mono">
              {closings.map(c => (
                <tr key={c.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 font-sans border-r border-slate-100 font-semibold">{c.closedBy === 'p-ahmed-ghaffar' ? 'Ahmed Partner' : 'Admin'}</td>
      ...
      <div className="bg-white p-5 border border-slate-100 rounded-xl shadow-sm">
        ...
        <p className="text-[10px] text-slate-400 mt-1 mb-4">Outstanding office balances owed as salary adjustments, freelancer wages, or Ahmed & Abu El-Fotouh quarterly split payables.</p>
