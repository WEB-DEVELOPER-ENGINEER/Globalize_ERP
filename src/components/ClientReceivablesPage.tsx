...
export const ClientReceivablesPage: React.FC<ClientReceivablesPageProps> = ({ isRtl, currentRole }) => {
  ...
  const handleGenerateInvoice = (client: Client) => {
    ...
    if (clientTasks.length === 0) {
      alert(isRtl 
        ? `⚠️ لا توجد أي مهام مكتملة غير مدفوعة للعميل: ${client.name} لإنشاء فاتورة لها!` 
        : `⚠️ No completed, unpaid tasks found for client: ${client.name} to compile into an invoice.`
      );
      return;
    }
  ...
  const handleOpenSettle = (rec: ClientReceivableRecord) => {
    if (currentRole !== 'owner' && currentRole !== 'accountant') {
      alert('Access Denied. Only Owners or Accountants can close client receivable sheets.');
      return;
    }
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 bg-slate-50">
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
                Active CRM Client Accounts Tree
              </h3>
            </div>
