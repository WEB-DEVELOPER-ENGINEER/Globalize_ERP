...
import { Task, Payment, Client, Profile, MonthlyClosing, TaskAssignment } from '../types';
...
export const DashboardOwner: React.FC<DashboardOwnerProps> = ({ isRtl, onNavigateTab }) => {
  ...
  // Let's compute actual revenues in June 2026/May 2026
  const junePayments = payments.filter(p => p.paymentDate.startsWith('2026-06'));
  const mathJuneRevenueEgp = junePayments.filter(p => p.paymentType === 'income').reduce((s, p) => s + p.amountEgp, 0);
  const mathJuneExpenseEgp = junePayments.filter(p => p.paymentType === 'expense').reduce((s, p) => s + p.amountEgp, 0);

  // Partner splits computation for last closed month (or simulation)
  const lastClosing = closings[closings.length - 1];
  const partnerAhmedShare = lastClosing ? lastClosing.partner1Share : 0;
  const partnerAbuShare = lastClosing ? lastClosing.partner2Share : 0;
  const latestClosedPeriod = lastClosing ? lastClosing.period : 'None';

  // Bank Positions Cash assets
  const bankPositions = [
    { title: 'Cashbox Khazina (خزينة)', value: 'EGP 34,500', color: 'border-emerald-200 bg-emerald-50/20' },
    { title: 'Bank SAIB EGP Account', value: 'EGP 132,400', color: 'border-indigo-200 bg-indigo-50/20' },
    { title: 'Bank NBE (البريد/الأهلي)', value: 'EGP 42,100', color: 'border-cyan-200 bg-cyan-50/20' },
    { title: 'Instapay (Mohamed)', value: 'EGP 19,800', color: 'border-purple-200 bg-purple-50/20' },
  ];
  ...
  const translatorsPerformance = profiles
    ...
    .map(p => {
      ...
      const completedLast30Days = translatorAsgs.filter(a => {
        ...
        const d = new Date(a.assignedAt || "2026-06-01");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-8 bg-zinc-950 text-white rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 border border-zinc-900 shadow-sm leading-tight">
        <div>
          <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-400 block mb-1">System Status</span>
          <h2 className="text-2xl font-light tracking-tight">
            {isRtl ? 'أهلاً بك شريك غلوبالايز الموقر' : 'Welcome Globalize System Partner'}
          </h2>
          <p className="text-xs text-zinc-400 mt-2 max-w-2xl leading-relaxed">
            {isRtl 
              ...
              : 'Audit active cash book balances, monthly historical Closings, translation costings, and direct partner dividend split of 50/50 Ahmed & Abu El-Fotouh.'}
          </p>
        </div>
        <div className="flex gap-2.5 shrink-0">
          <button 
            onClick={() => onNavigateTab('acc-closings')}
            className="px-4 py-2 bg-white hover:bg-zinc-200 text-zinc-950 text-xs font-semibold rounded-lg transition-all cursor-pointer font-sans"
          >
            {isRtl ? 'إجراء إغلاق مالي شهري' : 'Close Active Month'}
          </button>
          <button 
            onClick={() => onNavigateTab('tasks')}
            className="px-4 py-2 bg-zinc-900 hover:bg-zinc-850 text-white text-xs font-semibold rounded-lg transition-all border border-zinc-800 cursor-pointer"
          >
      ...
      {activeTab === 'financial' ? (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-6 rounded-xl border border-zinc-200 flex items-start gap-4">
              <div className="p-2.5 bg-zinc-950 text-white rounded-lg">
                <AlertCircle size={20} />
              </div>
              <div>
                <p className="text-[10px] font-semibold text-zinc-400 uppercase tracking-widest leading-none">
          ...
          {/* PARTNER NET DIVIDEND SHARING SHEET - Ahmed vs Abu El-Fotouh (50/50) */}
            ...
            <div className="bg-white p-6 rounded-xl border border-zinc-150 lg:col-span-2">
              ...
              <p className="text-xs text-zinc-400 leading-relaxed mt-3">
                {isRtl 
                  ...
                  : 'Upon finalizing standard monthly closings, all operational costs and staff/freelance wages are deducted. Net profit is allocated exactly 50% to Ahmed and 50% to Abu El-Fotouh.'}
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                {/* Ahmed share */}
                <div className="p-5 border border-zinc-150 bg-zinc-50/55 rounded-xl relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-1.5 h-full bg-zinc-900"></div>
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400">{isRtl ? 'الشريك الأول: أحمد عبد الغفار (50%)' : 'Ahmed Abdel Ghaffar (50%)'}</p>
                  <h4 className="text-2xl font-light tracking-tight text-zinc-900 mt-2.5">
                    EGP {partnerAhmedShare > 0 ? partnerAhmedShare.toLocaleString() : '323,320'}
                  </h4>
                ...
                <div className="p-5 border border-zinc-150 bg-zinc-50/55 rounded-xl relative overflow-hidden">
                  ...
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400">{isRtl ? 'الشريك الثاني: أبو الفتوح (50%)' : 'Abu El-Fotouh (50%)'}</p>
                  <h4 className="text-2xl font-light tracking-tight text-zinc-900 mt-2.5">
                    EGP {partnerAbuShare > 0 ? partnerAbuShare.toLocaleString() : '323,320'}
                  </h4>
                  <p className="text-[10px] text-zinc-450 text-zinc-400 mt-1">{isRtl ? 'صافي مستحق رصيد الدفعة المغلقة' : 'Net dividend allocation for latest closed cycle.'}</p>
                </div>
              </div>
            ...
            <div className="bg-white p-6 rounded-xl border border-zinc-150">
              ...
              <div className="space-y-3.5 mt-4">
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-semibold font-mono text-zinc-800 bg-zinc-900 text-white px-2.5 py-1 rounded">
                    {completedCount}
                  </span>
                </div>
              </div>
            ...
            <div className="bg-white p-6 rounded-xl border border-zinc-150">
              ...
              <div className="h-44 w-full flex items-end justify-between px-2 pt-4 relative">
                ...
                <div className="flex flex-col items-center flex-1 group">
                  ...
                  <div className="w-8 bg-zinc-950 rounded-t h-14"></div>
