...
export const QuotationsPage: React.FC<QuotationsPageProps> = ({ isRtl, currentRole }) => {
  ...
  const [qSourceLang, setQSourceLang] = useState('English');
  const [qTargetLang, setQTargetLang] = useState('Arabic');
  ...
  const handleRegisterQuotation = (e: React.FormEvent) => {
    ...
    if (currentRole !== 'owner' && currentRole !== 'sales' && currentRole !== 'admin') {
      alert('Access Denied. Only Owners, Sales, or Admins can issue quotes.');
      return;
    }
  ...
  const handleApproveQuote = (id: string) => {
    const quote = dbInstance.quotations.find(q => q.id === id);
    if (quote) {
      quote.status = 'approved';
      dbInstance.save();

      // Automatically register a Task based on approved quote!
      dbInstance.addTask({
        clientId: quote.clientId,
        clientPhone: clients.find(c => c.id === quote.clientId)?.phone || '+2010...',
        fileName: quote.fileName,
        serviceType: quote.serviceType,
        sourceLanguage: quote.sourceLanguage,
        targetLanguage: quote.targetLanguage,
        wordCount: quote.wordCount,
        pageCount: Math.ceil(quote.wordCount / 250),
        amountEgp: quote.amountEgp,
        amountAed: quote.amountAed,
      ...
      alert('Quotation Approved! Live translation workspace folder generated successfully.');
    }
  };
        ...
        <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-xl max-w-3xl mx-auto border-t-8 border-[#1B4F72] relative">
          ...
          <div className="absolute right-12 bottom-32 opacity-10 select-none hover:opacity-10 pointer-events-none transform rotate-12">
            <div className="w-36 h-36 border-4 border-double border-indigo-900 rounded-full flex flex-col items-center justify-center font-bold text-center text-indigo-900">
              ...
              <span className="text-[8px] font-mono leading-none mt-1">REG # Egypt 44102</span>
            </div>
          </div>
          ...
          <div className="flex justify-between pb-6 border-b-2 border-slate-150 border-slate-200 text-xs">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="p-1 px-2.5 rounded bg-[#1B4F72] text-[15px] font-black text-white shrink-0">GS</span>
                <span className="font-extrabold text-[#1B4F72] uppercase text-sm tracking-wider font-sans">Globalize System</span>
              </div>
              <p className="text-[10px] text-slate-400 font-medium">Certified Translation and Legal Review System</p>
              <p className="text-[10px] text-slate-450 mt-1 text-slate-400">Main Office: El-Tahrir Sq, Cairo, Egypt • Tel: +202 225910</p>
            </div>
          ...
          <div className="my-8 space-y-5 text-xs font-sans leading-relaxed">
            ...
            <p className="text-slate-600">
              We respectfully present this formal financial pricing quotation for certified translation services. Globalize System matches strictly peer-reviewed, state-accredited translators in legal, technical, and corporate specializations.
            </p>
                    ...
                    quotations.map(q => (
                      <tr key={q.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-5 py-3 text-right font-mono font-bold">
                          {q.amountEgp > 0 && <span>EGP {q.amountEgp.toLocaleString()}<br /></span>}
                          {q.amountAed > 0 && <span>AED {q.amountAed.toLocaleString()}<br /></span>}
