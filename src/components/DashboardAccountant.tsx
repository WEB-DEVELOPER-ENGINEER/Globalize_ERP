...
export const DashboardAccountant: React.FC<DashboardAccountantProps> = ({
  ...
  const [notes200, setNotes200] = useState<number>(100); // seed with some bills
  const [notes100, setNotes100] = useState<number>(85);
  const [notes50, setNotes50] = useState<number>(60);
  const [notes20, setNotes20] = useState<number>(45);
  const [notes10, setNotes10] = useState<number>(30);
  const [notes5, setNotes5] = useState<number>(20);
  const [notes1, setNotes1] = useState<number>(10);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm flex flex-col justify-between">
          ...
          <div className="text-[9px] text-slate-400 font-mono font-medium block text-center mt-3">
            Last Vault Count Check: 2026-06-09 10:46 UTC
          </div>
        </div>
      </div>
